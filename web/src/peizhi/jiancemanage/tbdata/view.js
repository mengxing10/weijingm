...
class TableDataManage extends Component {
    ...
    return (<div className='tbdata'>
      <ToastContainer ref="toastcontainer" className="toast-top-right"/>
        ...
        <div className="gfybiaogeguanli-choice">
          ...
          {data&&data['table']&&data['table']['status']&&data['table']['status']==1?<EditTableGw
  ...
  componentWillMount() {
   ...
   async  initPage(){
     ...
     const {data} = this.props
     await   operateTableData('ExcelTest1',0,'sel',data)
...
export default connect(state => state.jiancetbdatam, mapDispatchToProps)(TableDataManage);
