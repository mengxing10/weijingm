...
class ManageBiaoge extends Component {
          ...
          <div className="gfybiaogeguanli-choice-right">
            {
              biaogeadd?<div className="choice-div">
                <div className = "excel-upload">
                  <span className="name-span">导入表格:</span>
                <input name = "excelFile" ref="excelFile" type="file"  onChange={::this.setNewExcel}
                    accept=".csv, application/vnd.ms-excel, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"/>
                  <div onClick={::this.uploadExcel}>上传</div>
                </div>
  ...
  setNewExcel(ev) {
    ...
    this.setState({locfile:file})

  }


  async uploadExcel(){
    const {data,locfile} = this.state
    const {uploadExcel} = this.props
    await uploadExcel(data,locfile)
  ...
  async dataRefresh(){
    ...
    await selectColumnList(tableName)
...
export default connect(state => state.yewubiaogem, mapDispatchToProps)(ManageBiaoge);
