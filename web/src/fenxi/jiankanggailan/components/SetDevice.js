...
export default class SetDevice extends Component {
    ...
    render() {
        ...
        const {device,get_oldItem} = this.props;
        var oldItem = get_oldItem();
   ...
   var curNameID="feidiankongtiao";
   for(var i=0;i<stateOptions.length;i++)
       if(stateOptions[i].text ==  oldItem.name )
           curNameID = stateOptions[i].value;
  

const fields=[
{id:'0',key:'projectid',name:'项目id'},
...
kongtiaoshuibeng:[fields[2],fields[3],fields[4],fields[25],fields[5],fields[68],fields[76],fields[77],fields[78],fields[79],fields[26],fields[81]],
 ...
 //,fields[],fields[],fields[]
        ...
        return (
           ...
           <h4>名称</h4>
            <Input  disabled='true' value={oldItem["name"]}   />
            </div>

                       { filedOptions[curNameID].map((entry, index) => (  

                         <div className="device-item">
                         <h4>{entry.name}</h4>
                         <Input disabled='true' ref={entry.key} value={oldItem[entry.key]} onChange={this.changeValue.bind(this, entry.key)}/>
                         </div>
