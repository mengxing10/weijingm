...
export default class TanChuang extends Component {
  ...
  render() {
    ...
    const show = this.props.type=='dabiaolv'||this.props.type=='nengxiao'?false:true;
    const options = [
      {
        key: '实时数据',
        text: '实时数据',
  ...
  getCurrentData(starttime,endtime,types,statue){
      ...
      for(var i=0;i<=num;i++){
        ...
        var seriesList = {
          series:[{
            data:
            [
                [Math.floor(Math.random()*(max-min+1)+min), 9],[Math.floor(Math.random()*(max-min+1)+min), 6.95],[Math.floor(Math.random()*(max-min+1)+min), 7.58],
            ...
            markArea:
            {
              ...
              data: [[{xAxis:6,yAxis:5},{xAxis:12,yAxis:10}]]
            }
          }]}
    ...
    if(statue=='nengxiao'){
      ...
      for(var i=0;i<=num;i++){
        ...
        var seriesList = {
          series:[
            {data:[2,4,6,8,6,4,2],type:'line',symbol:'none'},
            {data:[6,8,10,12,10,8,6 ],type:'line',symbol:'none'},
            {data:[,,,,,,10,12,14,16,14,12,10],type:'line',symbol:'none'},
            {data:[,,,,,,6,8,10,12,10,8,6],type:'line',symbol:'none'},
            {name:"101水泵",data:[[2, 9],[Math.floor(Math.random()*(max-min+1)+min), 6.95],[Math.floor(Math.random()*(max-min+1)+min), 7.58],
                [9.0, 8.81],[11.0, 8.33],[Math.floor(Math.random()*(max-min+1)+min), 9.96],[6.0, 7.24],[4.0, 4.26],[Math.floor(Math.random()*(max-min+1)+min), 10.84],[7.0, 4.82],[5.0, 5.68]
            ],type: 'scatter'},
            {name:"102水泵",data:[[2, 5],[Math.floor(Math.random()*(max-min+1)+min), 6.95],[Math.floor(Math.random()*(max-min+1)+min), 7.58],
                [9.0, 8.81],[11.0, 8.33],[Math.floor(Math.random()*(max-min+1)+min), 9.96],[6.0, 7.24],[4.0, 4.26],[Math.floor(Math.random()*(max-min+1)+min), 10.84],[7.0, 4.82],[5.0, 5.68]
            ],type: 'scatter'},
            {name:"131水泵",data:[[2,7],[Math.floor(Math.random()*(max-min+1)+min), 6.95],[Math.floor(Math.random()*(max-min+1)+min), 7.58],
                [9.0, 8.81],[11.0, 8.33],[Math.floor(Math.random()*(max-min+1)+min), 9.96],[6.0, 7.24],[4.0, 4.26],[Math.floor(Math.random()*(max-min+1)+min), 10.84],[7.0, 4.82],[5.0, 5.68]
            ],type: 'scatter'},
            {name:"104水泵",data:[[2,1],[Math.floor(Math.random()*(max-min+1)+min), 6.95],[Math.floor(Math.random()*(max-min+1)+min), 7.58],
                [9.0, 8.81],[11.0, 8.33],[Math.floor(Math.random()*(max-min+1)+min), 9.96],[6.0, 7.24],[4.0, 4.26],[Math.floor(Math.random()*(max-min+1)+min), 10.84],[7.0, 4.82],[5.0, 5.68]
            ],type: 'scatter'}
          ]}
          ydata0.push(seriesList);
  ...
  handleClickStart(type){
    var types = 'shishi';
    var shuju = {};
    if(type=='dabiaolv'||type=='nengxiao'){
      types = 'ri';
  ...
  getOptions(type,shuju){
    ...
    if(type=="dabiaolv"){
      ...
      baseOption.tooltip.formatter = function(params) {
		            			                return params.value[0]+options.xAxis.name + '<br/> '+params.value[1]+options.yAxis.name;
		            			        };
    ...
    if(type=='zuoyelv'){
      XiangMuDaBiaoLv_tc = {
        ...
        series:{
          lines:[
                 {name:"101水泵",data:shuju.ydata1, areaStyle: {normal: {color:"#73eecc"}},symbol:'emptyCircle'  },
                 {name:"102水泵",data:shuju.ydata2, areaStyle: {normal: {color:"#00e099"}},symbol:'emptyCircle'   },
                 {name:"131水泵",data:shuju.ydata3, areaStyle: {normal: {color:"#facc17"}},symbol:'emptyCircle'  },
                 {name:"104水泵",data:shuju.ydata4, areaStyle: {normal: {color:"#5c67e5"}},symbol:'emptyCircle'   },
    ...
    if(type=='jiankangdu'){
      XiangMuDaBiaoLv_tc = {
        ...
        series:{
            lines:[
              {name:"101水泵",data:shuju.ydata0,symbol:'emptyCircle' },
              {name:"102水泵",data:shuju.ydata1 },
              {name:"131水泵",data:shuju.ydata2 },
              {name:"104水泵",data:shuju.ydata3 }
            ],
         }
      };
      return GeneralCharts.theLineBar(XiangMuDaBiaoLv_tc);
    ...
    if(type=='nengxiao'){
      ...
      var options = {
        ...
        series: {
          lines:[
            {name:"101水泵",data:[2,4,6,8,6,4,2].map(item=>(Math.round(item*(1+Math.random()/5)*10)/10))},
            {name:"102水泵",data:[6,8,10,12,10,8,6 ].map(item=>(Math.round(item*(1+Math.random()/5)*10)/10))},
            {name:"131水泵",data:[,,,,,,10,12,14,16,14,12,10]},
            {name:"104水泵",data:[,,,,,,6,8,10,12,10,8,6]}
         ]
        },
      };
      var baseOption =  GeneralCharts.theLineBar(options);
  ...
  handleChangeStart(dateString, { dateMoment, timestamp}) {
      var shujuType = '';
      if(this.props.type=='dabiaolv'||this.props.type=='nengxiao'){
        shujuType='ri'
      }else{
